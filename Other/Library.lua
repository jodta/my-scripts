...
local Library = { Toggled = true, Accent = Color3.fromRGB(160, 60, 255), _blockDrag = false, ForceMobile = true }
...
function Library:CreateWindow(title)
    ...
    local isMobile = Library.ForceMobile or (UIS.TouchEnabled and not UIS.KeyboardEnabled)
    ...
    local function updateScale()
        ...
        -- FIX: More aggressive scaling for mobile so the window fits the screen
        local refX = isMobile and 860 or 800
        local refY = isMobile and 520 or 550
        local scaleFactor = math.min(view.X / refX, view.Y / refY, 1)
        -- FIX: Slightly reduce mobile scale multiplier (was 1.1 which made it too large)
        if isMobile then scaleFactor = scaleFactor * 0.95 end
    ...
    -- FIX: Shrink title label on mobile so it doesn't eat into subtab space
    Create("TextLabel", {
        Parent = Header,
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 16, 0, 0),
        Size = UDim2.new(0, isMobile and 100 or 180, 1, 0),
        Font = "GothamBold",
    ...
    -- FIX: Use a ScrollingFrame for SubTabBar so tabs don't get clipped on mobile.
    -- Horizontal scroll (invisible scrollbar) lets all tabs remain accessible.
    local subTabOffset = isMobile and 116 or 200
    local SubTabBar = Create("ScrollingFrame", {
        Parent = Header,
        BackgroundTransparency = 1,
        Position = UDim2.new(0, subTabOffset, 0, 0),
        Size = UDim2.new(1, -subTabOffset, 1, 0),
    ...
    Create("UIListLayout", { Parent = SubTabBar, FillDirection = "Horizontal", Padding = UDim.new(0, isMobile and 10 or 16), VerticalAlignment = "Center" })
    ...
    -- ── Mobile toggle button (touch devices only) ────────────────────────────
    local MobileBtn
    if isMobile then
    ...
    function Window:CreateTab(name, iconName)
        ...
        function Tab:CreateSubTab(stName, stIconName)
            ...
            -- FIX: Use AutomaticSize properly; frame is sized by its children
            ...
            local SIco = Create("ImageLabel", {
                ...
                -- FIX: Slightly smaller icon on mobile to save space
            ...
            local SText = Create("TextLabel", {
                ...
                -- FIX: Smaller text on mobile so all tabs fit
            ...
            Create("UIListLayout", { Parent = SPage, Padding = UDim.new(0, 14), HorizontalAlignment = "Center", Name = "Layout" })
            Create("UIPadding", { Parent = SPage, PaddingTop = UDim.new(0, 14), PaddingLeft = UDim.new(0, 18), PaddingRight = UDim.new(0, 18), PaddingBottom = UDim.new(0, 24) })
            ...
            function SubTab:CreateSection(secName)
                ...
                Create("UIPadding", { Parent = Content, PaddingTop = UDim.new(0, 4) })

                local L = Create("UIListLayout", { Parent = Content, Padding = UDim.new(0, 10) })
